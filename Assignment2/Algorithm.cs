...
namespace GeneticAlgorithm
{
    public class GeneticAlgorithm<Ind>
    {
        ...
        public Tuple<Ind,Tuple<double,double>> Run(Func<Ind> createIndividual, Func<Ind, double> computeFitness, Func<Ind[], double[], Func<Tuple<Ind, Ind>>> selectTwoParents,
            Func<Tuple<Ind, Ind>, Tuple<Ind, Ind>> crossover, Func<Ind, double, Ind> mutation)
        {
            ...
            for (int generation = 0; generation < numIterations; generation++)
            {
                ...
                for (int newInd = startIndex; newInd < populationSize; newInd++)
                {
                    ...
                    if (newInd < populationSize) //there is still space for the second child inside the population
            ...
            var finalAverageFitness = finalFitnesses.Average();
            return currentPopulation
                .Select((individual, index) => new Tuple<Ind, Tuple<double, double>>(individual, Tuple.Create(finalFitnesses[index], finalAverageFitness)))
